...
class Main:
        ...
        if self.pseudo != "":
            try:
                ...
                if "connect#" in msg_recu:
                    ...
                    self.image["tache3"] = PhotoImage(file=self.dir + '\\image\\tache3.png')
                    self.image["tache2"] = PhotoImage(file=self.dir + '\\image\\tache2.png')
                    self.image["tache1"] = PhotoImage(file=self.dir + '\\image\\tache1.png')
                    ...
                    self.image["fondattente"] = PhotoImage(
                        file=self.dir + '\\image\\Frame avec bouton file dattente.png')
                    self.image["fondattente2"] = PhotoImage(
                        file=self.dir + '\\image\\Frame avec bouton(partie en cours).png')
                    ...
                    self.image["piege"] = PhotoImage(file=self.dir + '\\image\\piege.png')
                    ...
                    self.perso = self.listforfenetre["canvasgame"].create_image(-100, 100,
                                                                                image=self.image["PersoMenu"])
                    self.perso2 = self.listforfenetre["canvasgame"].create_image(180, 500,
                                                                                 image=self.image["Perso2Menu"])
                    ...
                    self.listforfenetre["listtchat"] = Listbox(self.listforfenetre["frametchat"], bd=0, width=1000,
                                                               yscrollcommand=self.listforfenetre["scrolltchat"].set)
                    self.listforfenetre["listtchat"].pack()
                    self.listforfenetre["scrolltchat"].config(command=self.listforfenetre["listtchat"].yview)
                    string = StringVar()
                    self.listforfenetre["entrytchat"] = Entry(self.listforfenetre["frametchat"], width=1200,
                                                              textvariable=string, bg='ivory3', fg='maroon')
    ...
    def callback(self, event):
        if self.menu == "principale":
            ...
            if 645 <= event.x <= 1060 and 247 <= event.y <= 300:
                ...
                self.menu = "aide1"
            if 380 < event.x < 806 and 97 < event.y < 167:
                self.sender.publish(PacketInfoGame().init(self))
            if 120 <= event.x <= 543 and 252 <= event.y <= 303:
            ...
            if 466 <= event.x <= 749 and 111 <= event.y <= 224:
                self.listforfenetre["canvasgame"].create_image(600, 250, image=self.image["parafleche"])
    ...
    def key(self, event):
        if event.keysym == "Return" and self.listforfenetre["entrytchat"].get() != '':
            if self.listforfenetre["entrytchat"].get() == "/ms":
                ...
                self.sender.publish(
                    PacketChat().init(self, self.pseudo + " : " + self.listforfenetre["entrytchat"].get()))
    ...
    def encode(self, value):
        value = value.replace("À", "*%00%").replace("Á", "*%01%").replace("Â", "*%02%").replace("Ã", "*%03%").replace(
            "Ä", "*%04%").replace("Å", "*%05%").replace("Æ", "*%06%").replace("Ç", "*%07%").replace("È",
                                                                                                    "*%08%").replace(
            "É", "*%09%").replace("Ê", "*%0A%").replace("Ë", "*%0B%").replace("Ì", "*%0C%").replace("Í",
                                                                                                    "*%0D%").replace(
            "Î", "*%0E%").replace("Ï", "*%0F%")
        value = value.replace("Ð", "*%10%").replace("Ñ", "*%11%").replace("Ò", "*%12%").replace("Ó", "*%13%").replace(
            "Ô", "*%14%").replace("Õ", "*%15%").replace("Ö", "*%16%").replace("Ø", "*%17%").replace("Œ",
                                                                                                    "*%18%").replace(
            "Š", "*%19%").replace("þ", "*%1A%").replace("Ù", "*%1B%").replace("Ú", "*%1C%").replace("Û",
                                                                                                    "*%1D%").replace(
            "Ü", "*%1E%").replace("Ý", "*%1F%")
        value = value.replace("Ÿ", "*%20%").replace("à", "*%21%").replace("á", "*%22%").replace("â", "*%23%").replace(
            "ã", "*%24%").replace("ä", "*%25%").replace("å", "*%26%").replace("æ", "*%27%").replace("ç",
                                                                                                    "*%28%").replace(
            "è", "*%29%").replace("é", "*%2A%").replace("ê", "*%2B%").replace("ë", "*%2C%").replace("ì",
                                                                                                    "*%2D%").replace(
            "í", "*%2E%").replace("î", "*%2F%")
        value = value.replace("ï", "*%30%").replace("ð", "*%31%").replace("ñ", "*%32%").replace("ò", "*%33%").replace(
            "ó", "*%34%").replace("ô", "*%35%").replace("õ", "*%36%").replace("ö", "*%37%").replace("ø",
                                                                                                    "*%38%").replace(
            "œ", "*%39%").replace("š", "*%3A%").replace("Þ", "*%3B%").replace("ù", "*%3C%").replace("ú",
                                                                                                    "*%3D%").replace(
            "û", "*%3E%").replace("ü", "*%3F%")
        value = value.replace("ý", "*%40%").replace("ÿ", "*%41%").replace("¢", "*%42%").replace("ß", "*%43%").replace(
            "¥", "*%44%").replace("£", "*%45%").replace("™", "*%46%").replace("©", "*%47%").replace("®",
                                                                                                    "*%48%").replace(
            "ª", "*%49%").replace("×", "*%4A%").replace("÷", "*%4B%").replace("±", "*%4C%").replace("²",
                                                                                                    "*%4D%").replace(
            "³", "*%4E%").replace("¼", "*%4F%")
        value = value.replace("½", "*%50%").replace("¾", "*%51%").replace("µ", "*%52%").replace("¿", "*%53%").replace(
            "¶", "*%54%").replace("·", "*%55%").replace("¸", "*%56%").replace("º", "*%57%").replace("°",
                                                                                                    "*%58%").replace(
            "¯", "*%59%").replace("§", "*%5A%").replace("…", "*%5B%").replace("¤", "*%5C%").replace("¦",
                                                                                                    "*%5D%").replace(
            "≠", "*%5E%").replace("¬", '*%5F%')
    ...
    def decode(self, value):
        value = value.replace("*%00%", "À").replace("*%01%", "Á").replace("*%02%", "Â").replace("*%03%", "Ã").replace(
            "*%04%", "Ä").replace("*%05%", "Å").replace("*%06%", "Æ").replace("*%07%", "Ç").replace("*%08%",
                                                                                                    "È").replace(
            "*%09%", "É").replace("*%0A%", "Ê").replace("*%0B%", "Ë").replace("*%0C%", "Ì").replace("*%0D%",
                                                                                                    "Í").replace(
            "*%0E%", "Î").replace("*%0F%", "Ï")
        value = value.replace("*%10%", "Ð").replace("*%11%", "Ñ").replace("*%12%", "Ò").replace("*%13%", "Ó").replace(
            "*%14%", "Ô").replace("*%15%", "Õ").replace("*%16%", "Ö").replace("*%17%", "Ø").replace("*%18%",
                                                                                                    "Œ").replace(
            "*%19%", "Š").replace("*%1A%", "þ").replace("*%1B%", "Ù").replace("*%1C%", "Ú").replace("*%1D%",
                                                                                                    "Û").replace(
            "*%1E%", "Ü").replace("*%1F%", "Ý")
        value = value.replace("*%20%", "Ÿ").replace("*%21%", "à").replace("*%22%", "á").replace("*%23%", "â").replace(
            "*%24%", "ã").replace("*%25%", "ä").replace("*%26%", "å").replace("*%27%", "æ").replace("*%28%",
                                                                                                    "ç").replace(
            "*%29%", "è").replace("*%2A%", "é").replace("*%2B%", "ê").replace("*%2C%", "ë").replace("*%2D%",
                                                                                                    "ì").replace(
            "*%2E%", "í").replace("*%2F%", "î")
        value = value.replace("*%30%", "ï").replace("*%31%", "ð").replace("*%32%", "ñ").replace("*%33%", "ò").replace(
            "*%34%", "ó").replace("*%35%", "ô").replace("*%36%", "õ").replace("*%37%", "ö").replace("*%38%",
                                                                                                    "ø").replace(
            "*%39%", "œ").replace("*%3A%", "š").replace("*%3B%", "Þ").replace("*%3C%", "ù").replace("*%3D%",
                                                                                                    "ú").replace(
            "*%3E%", "û").replace("*%3F%", "ü")
        value = value.replace("*%40%", "ý").replace("*%41%", "ÿ").replace("*%42%", "¢").replace("*%43%", "ß").replace(
            "*%44%", "¥").replace("*%45%", "£").replace("*%46%", "™").replace("*%47%", "©").replace("*%48%",
                                                                                                    "®").replace(
            "*%49%", "ª").replace("*%4A%", "×").replace("*%4B%", "÷").replace("*%4C%", "±").replace("*%4D%",
                                                                                                    "²").replace(
            "*%4E%", "³").replace("*%4F%", "¼")
        value = value.replace("*%50%", "½").replace("*%51%", "¾").replace("*%52%", "µ").replace("*%53%", "¿").replace(
            "*%54%", "¶").replace("*%55%", "·").replace("*%56%", "¸").replace("*%57%", "º").replace("*%58%",
                                                                                                    "°").replace(
            "*%59%", "¯").replace("*%5A%", "§").replace("*%5B%", "…").replace("*%5C%", "¤").replace("*%5D%",
                                                                                                    "¦").replace(
            "*%5E%", "≠").replace("*%5F%", "¬")
